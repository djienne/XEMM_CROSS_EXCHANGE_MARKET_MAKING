...
class HyperliquidConnector extends EventEmitter {
   ...
   */
  async getOpenOrders(user = null) {
    user = user || this.wallet;

    if (!user) {
      throw new Error('User address required to get open orders');
    }
    ...
    try {
      ...
      return data.map(order => ({
        orderId: order.oid,
        coin: order.coin,
        side: order.side === 'B' ? 'buy' : 'sell', // B = Bid/Buy, A = Ask/Sell
        limitPrice: parseFloat(order.limitPx),
      ...
      console.error('[Hyperliquid] Error fetching open orders:', error.message);
      throw error;
    }
  }
   ...
   */
  async getUserFills(user = null, startTime = null) {
    user = user || this.wallet;

    if (!user) {
      throw new Error('User address required to get user fills');
    }
    ...
    try {
      ...
      return data.map(fill => ({
        ...
        direction: fill.dir, // e.g., "Open Long", "Close Short"
      ...
      console.error('[Hyperliquid] Error fetching user fills:', error.message);
      throw error;
    }
  }

  /**
   * Get user state (positions, balances, margins) - full account state
   * @param {string} user - User address (defaults to configured wallet)
   * @returns {Promise<object>} User state with positions and balances
   */
  async getUserState(user = null) {
    user = user || this.wallet;

    if (!user) {
      throw new Error('User address required to get user state');
    }

    try {
      // Use clearinghouseState for main account state
      const response = await fetch(this.restUrl, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
      ...
      const positions = (data.assetPositions || []).map(pos => ({
        ...
        raw: pos
      })).filter(pos => pos.size > 0); // Only include non-zero positions
      ...
      console.error('[Hyperliquid] Error fetching user state:', error.message);
      throw error;
    }
  }
}
